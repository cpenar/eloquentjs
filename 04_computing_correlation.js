...
(function() {
  var JOURNAL, addEntry, correlation, correlations, event, gatherCorrelations, hasEvent, journal, map, phi, storePhi, tableFor,
  ...
  addEntry = function(events, didITurnIntoASquirrel) {
    var local;
    local = {
      events: events,
      squirrel: didITurnIntoASquirrel
    };
    return journal.push(local);
  };
  ...
  hasEvent = function(event, entry) {
    return (entry.events.indexOf(event)) !== -1;
  };

  tableFor = function(event, journal) {
    var entry, i, index, j, ref, table;
    table = [0, 0, 0, 0];
    for (i = j = 0, ref = journal.length - 1; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
      entry = journal[i];
      index = 0;
  ...
  storePhi = function(event, phi) {
    return map[event] = phi;
  };

  gatherCorrelations = function(journal) {
    var entry, event, events, i, j, k, phis, ref, ref1;
    phis = {};
    for (entry = j = 0, ref = journal.length - 1; 0 <= ref ? j <= ref : j >= ref; entry = 0 <= ref ? ++j : --j) {
      events = journal[entry].events;
      for (i = k = 0, ref1 = events.length - 1; 0 <= ref1 ? k <= ref1 : k >= ref1; i = 0 <= ref1 ? ++k : --k) {
        event = events[i];
        if (!(indexOf.call(phis, event) >= 0)) {
  ...
  for (event in correlations) {
    correlation = correlations[event];
